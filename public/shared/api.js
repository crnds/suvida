...
const Api = {
  ...
  editTemplateEntry: (id, weekday, start_minutes, location_id) => apiFetch(`/api/admin/template/${id}`, { method: 'PATCH', body: { weekday, start_minutes, location_id } }),
  removeTemplateEntry: (id) => apiFetch(`/api/admin/template/${id}`, { method: 'DELETE' }),
  listLocations: () => apiFetch('/api/admin/locations'),
  addLocation: (title, title_th) => apiFetch('/api/admin/locations', { method: 'POST', body: { title, title_th } }),
  editLocation: (id, title, title_th) => apiFetch(`/api/admin/locations/${id}`, { method: 'PATCH', body: { title, title_th } }),
